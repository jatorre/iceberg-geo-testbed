...
| **Snowflake 10.19.100 (GCP-EU)** | **L3** — pruning via manifest `record_count` (`bytes_scanned=0`) | **L3** — same; Snowflake variant access `bbox:xmin::FLOAT` | **❓ untested** — Snowflake correctly rejects our V3 fixture as `incomplete state` because *our* writer pairs V3 metadata.json with V2 manifest avro (pyiceberg 0.11.1 limitation). Other engines accept the hybrid; Snowflake doesn't, which is the spec-correct behavior. Snowflake's claimed `full` V3 geometry support per icebergmatrix.org is unverified by us, **not invalidated**. Verifying it needs a Snowflake-managed V3 table or a third-party V3 manifest writer. Earlier 091369 we hit was an IAM trap (`storage.buckets.get` missing); Snowflake support diagnosed it. Full detail in [engines/snowflake/README.md](engines/snowflake/README.md). |
