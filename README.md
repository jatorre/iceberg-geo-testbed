...
| **Snowflake**          | ⏸ blocked | ⏸ | ⏸ | Two accounts tried. CARTO dev (shared): can't `CREATE EXTERNAL VOLUME` from `TEST_ROLE`. Personal trial on GCP-EU: have ACCOUNTADMIN and external volume verifies clean, but every `CREATE ICEBERG TABLE` — *even Snowflake-managed* — fails with `091369: Query needs to be retried to setup external volume`. See [engines/snowflake/README.md](engines/snowflake/README.md). |
| **Apache Sedona**      | ❓ | ❓ | ❓ — also the ground-truth implementation we'll diff our manifest avro against | Runner in `engines/sedona/`. |
