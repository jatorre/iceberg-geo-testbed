...
| **Snowflake 10.19.100 (GCP-EU)** | **L3** — pruning via manifest `record_count` (`bytes_scanned=0`) | **L3** — same; Snowflake variant access `bbox:xmin::FLOAT` | **L0** — registers `v2_geo_convention` cleanly at L3 too. V3 metadata rejected with `Iceberg table 'V3_GEOMETRY' is V3 but is in an incomplete state. Please complete the upgrade before creating an iceberg table.` Root cause: pyiceberg 0.11.1 writes V2 manifest avros while our metadata.json claims V3; Snowflake's V3 reader is strict enough to catch the inconsistency (Polaris/Iceberg-Spark accept the hybrid). The 091369 we hit earlier was an IAM trap — `roles/storage.objectAdmin` doesn't grant `storage.buckets.get`. Snowflake support diagnosed this. Documented in [engines/snowflake/README.md](engines/snowflake/README.md). |
