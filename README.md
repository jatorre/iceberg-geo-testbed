...
|---|---|---|---|
| **DuckDB 1.5.3**       | **L3** — prunes to 1/10 files | **L2** — correct, but no file pruning (struct-field gap) | **L2** — schema parses, `COUNT(*)` works, `SELECT geom` returns typed geometries and `ST_AsText(geom)` produces clean WKT (the GeoParquet 2.0 native typing in our fixture made this work). Only remaining gap: `WHERE ST_Intersects(...)` crashes the manifest-bound deserializer (`IcebergValue::DeserializeValue` has no GEOMETRY branch yet). Filed at [duckdb-iceberg#1002](https://github.com/duckdb/duckdb-iceberg/issues/1002). See [docs/duckdb-gap.md](docs/duckdb-gap.md). |
