...
| **Snowflake 10.19.100 (GCP-EU)** | **L3** — pruning via manifest `record_count` (`bytes_scanned=0`) | **L3** — same; Snowflake variant access `bbox:xmin::FLOAT` | **L3** *via Snowflake-managed write path* — `CREATE ICEBERG TABLE … GEOMETRY ICEBERG_VERSION=3` works; spatial predicate returns correct 1000 rows for the California envelope; `bytes_scanned=0` confirms manifest geometry-bound pruning. Snowflake is the first engine in this testbed delivering V3 geometry end-to-end. ⚠️ The *unmanaged* path (Snowflake reading externally-written V3 metadata) is stricter — even structurally-matching V3 fixtures get rejected unless their parquet files contain Snowflake's row-lineage metadata columns. See [engines/snowflake/README.md](engines/snowflake/README.md) for the full diagnostic + Path-1 experiment details. |
