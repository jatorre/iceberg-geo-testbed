...
| **Dataproc Serverless 2.3** (Spark 3.5.3, GCP) | ❌ — bundled `iceberg-spark-runtime` raises `UnsupportedOperationException: Cannot convert unknown type to Spark: geometry` on `spark.read.format("iceberg").load(...)`. Confirms the upstream Spark-Iceberg gap; the matrix's "Unknown" cell now fills as L0. | ❌ — blocked by N1 | ❌ — blocked by N1 | ❌ — blocked by N1 | ❓ |
| **EMR Serverless 7.13** (Spark 3.5.6-amzn-2, AWS) | ❌ — same `UnsupportedOperationException: Cannot convert unknown type to Spark: geometry` from the bundled `iceberg-spark-runtime`. icebergmatrix.org lists "EMR (8.0 Spark): Full" for V3 Geometry, but `emr-8.x` doesn't exist in `aws emr list-release-labels` (latest is `emr-7.13.0`) — that cell appears to be forward-looking or aspirational; the actually-testable EMR today is L0. | ❌ — blocked by N1 | ❌ — blocked by N1 | ❌ — blocked by N1 | ❓ |
